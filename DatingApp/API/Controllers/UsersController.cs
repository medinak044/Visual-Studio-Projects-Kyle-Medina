...
namespace API.Controllers
{
    ...
    public class UsersController : BaseApiController
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<MemberDto>>> GetUsers([FromQuery]UserParams userParams) // Gets multiple users
        {
        ...
        {
            var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername()); // Gets username value from the user's token (more reliable and secure than obtaining the data from the user themself)

            _mapper.Map(memberUpdateDto, user); // Maps values from one object to another

            _unitOfWork.UserRepository.Update(user); // Flag that user has been updated by Entity Framework
        ...
        public async Task<ActionResult> SetMainPhoto(int photoId)
        {
            ...
            if (await _unitOfWork.Complete()) return NoContent(); // Don't need to return anything back in this request
